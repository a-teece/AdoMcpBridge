...
internal sealed class UpdateCommentTool : ICustomMcpTool
{
    /// <summary>
    ...
    private readonly ILogger<UpdateCommentTool> _logger;

    public UpdateCommentTool(IBlobSlotStore blobs, IAdoRestClient ado, ILogger<UpdateCommentTool> logger)
    {
        _blobs = blobs;
        _ado = ado;
        _logger = logger;
    ...
    public string Name => "ado_bridge_update_comment";
    public object? Annotations => new { readOnlyHint = false };

    public string Description =>
        "Write operations: Updates (edits) an existing comment on an Azure DevOps work item, by comment id. " +
        "Complements ado_bridge_add_comment (the upstream comment-write tool is blocked by the bridge, so this is " +
        "the only way to edit a comment through the bridge). " +
        ...
        "(A comment saved without an explicit format defaults to HTML in ADO, which shows a Markdown body as " +
    ...
    public object InputSchema => new
    {
        ...
        properties = new
        {
            ...
            commentId = new { type = "integer", description = "Id of the existing comment to update." },
            ...
            format = new { type = "string", @enum = new[] { "markdown", "html" }, description = "REQUIRED — no default is assumed. 'markdown': body stored as native Markdown (ADO renders it). 'html': body stored as HTML. A comment saved without a format defaults to HTML and shows Markdown as raw syntax." },
        },
        required = new[] { "organization", "project", "workItemId", "commentId", "format" },
    };
    ...
    public async Task<McpToolResult> InvokeAsync(JsonElement arguments, CancellationToken ct)
    {
        ...
        if (!isMarkdown && !isHtml)
        {
            ...
            _logger.LogWarning("ado_bridge_update_comment: rejected — format={Received}", received);
        ...
        _logger.LogInformation(
            "ado_bridge_update_comment: comment {CommentId} on WI {Id} in {Org}/{Project} via {Source}",
            commentId, workItemId, org, project, slotId is null ? "inline" : "slot");
        ...
        try
        {
            updated = await _ado.UpdateWorkItemCommentAsync(org, project, workItemId, commentId, body, isMarkdown, ct)
        ...
        int? updatedCommentId = updated.TryGetProperty("id", out var idEl) && idEl.ValueKind == JsonValueKind.Number
            ? idEl.GetInt32()
            : commentId;

        _logger.LogInformation(
            "ado_bridge_update_comment: WI {Id} status=UPDATED commentId={CommentId} chars={Chars} format={Format}",
            workItemId, updatedCommentId, body.Length, isMarkdown ? "markdown" : "html");
